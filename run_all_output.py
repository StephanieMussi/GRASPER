...
def main():
    ...
    pdf_paths = ["source/Imagenet training in minutes.pdf",
                "source/Knowledge-Based Version Incompatibility Detection.pdf",
                "source/Large batch optimization for deep learning  Training bert in 76 minutes.pdf",
                "source/Large batch training of convolutional networks.pdf",
                "source/The Three Pillars of Machine Programming.pdf",
                "source/XCon Learning with Experts for Fine-grained Category Discovery.pdf",]
    
    for pdf_path in pdf_paths:
        paper_title = get_paper_title(pdf_path)
        for summarizer in ["bart", "t5", "llama2", "mistral"]:
            for converter in ["gpt3", "llama2", "mistral"]:
                ...
                anim_thread.start()
                final_summary = summarize_text(long_text, summarizer)
                summarizing = False
                anim_thread.join() 
                ...
                conv_thread.start()
                message = convert_content(final_summary, converter, args.openaikey)
                converting = False
                conv_thread.join() 

                """Step4. Generate presentation slides from content"""
                template_path = './source/template.pptx'
                save_path = './output/'+summarizer+' '+converter+'/'+paper_title+'.pptx'
                global generating
                generating = True
